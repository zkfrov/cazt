...
import { Point, type GrumpkinScalar } from '@aztec/aztec.js/fields';
import { 
  FunctionSelector, 
  EventSelector, 
  NoteSelector, 
...
import { keccak256, sha256ToField, poseidon2Hash, poseidon2HashWithSeparator, pedersenHash, Aes128 } from '@aztec/foundation/crypto';
...
import { computePreaddress, computeAddress, deriveMasterIncomingViewingSecretKey, computeAddressSecret } from '@aztec/stdlib/keys';
...
export class AztecUtilities {
   ...
   * 
   * @param params JSON string with: { 
   *   ciphertext: string[], // Array of 17 field values (hex strings)
   *   recipientAddress: string, // Complete address of the recipient
   *   recipientSecretKey: string, // Secret key (Fr) of the recipient
   * }
   * @returns Promise<string[]> - Array of decrypted field values (hex strings)
   */
  static async decryptRawPrivateLog(params: string): Promise<string[]> {
    ...
    const ciphertext = ciphertextFields.map((f: string) => this.stringToFr(f));
    ...
    const recipientIvskM = deriveMasterIncomingViewingSecretKey(this.stringToFr(recipientSecretKey));
